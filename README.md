# saraswati-awasthi
# 👩‍🔬  Saraswati Awasthi

**PhD scholar | Environmental Microbiome · AMR  · Metagenomics · Bioinformatics**  
📍 New Delhi, India

I am a researcher and bioinformatician specializing in environmental microbiome analysis, antimicrobial resistance (AMR) surveillance, and metagenomic data analysis. My work focuses on understanding microbial communities across urban and rural environments — from house floors and wastewater drains to lakes and Hot springs — with a particular emphasis on AMR gene dynamics and metagenome-assembled genomes (MAGs).

---

## 🔬 Research Interests

- Environmental & built-environment microbiomes (houses, lakes, wastewater, Hot spring)
- Antimicrobial resistance (AMR) surveillance and ARG profiling
- Shotgun metagenomics · 16S amplicon sequencing
- Metagenome-assembled genomes (MAGs): assembly, binning, annotation
- Comparative genomics · Phylogenetics · Biosynthetic gene clusters (BGCs)
- Workflow automation (Snakemake · Nextflow)

---

## 🧬 Current Projects

### 🏠 House Microbiome Study (PhD Research)
> Profiling indoor microbial communities across 76 rural and urban households (348 samples)

- **Scope:** 8 environmental and socioeconomic factors — landscape (rural/urban), socioeconomic status, cattle presence, floor type, pet presence, disinfection frequency, water source, and sampling location
- **Read-level:** Microbiome diversity and ARG diversity analyses across all factors
- **MAG-level:** dRep species-level genome bins (SGBs), ARG–MGE associations, metabolic reconstruction
- **Tools:** MetaPhlAn4,HUMAnN3, MetaSPAdes, MetaBAT2, dRep, CheckM2, DRAM, R

---

### 🏚️ House Floor Microbiome — New Delhi (Pilot)
> MAG recovery and characterization from house-floor samples (90 samples, 10 houses)

- Recovered high-quality MAGs from urban house floors
- **E. coli storyline:** Phylogenetic comparison with 72 pathogenic *E. coli* strains; ARG and virulence factor profiling; identification of potentially pathogenic MAGs
- **Tools:** MetaSPAdes, MetaBAT2, CheckM, Prokka, GTDB-Tk, ABRicate, R

---

### 🌊 Lake Microbiome — New Delhi (Shotgun Metagenomics)
> Metagenomic profiling of 60 urban lakes with chemical driver analysis
- **Tools:** MetaSPAdes, MetaBAT2, dRep, CheckM2, Prokka, DRAM, R

---

### 🚰 Wastewater Drain Microbiome
> Environmental microbiome analysis from urban drain samples

- Taxonomic profiling and ARG characterization from wastewater drain communities
- Integration with broader AMR surveillance framework
- **Tools:** Kraken2, Bracken, AMRFinderPlus, R

---

### 🦠 AMR Surveillance — Rural & Urban Environments, New Delhi
> Comparative AMR profiling across house and water body environments

- Rural vs. urban AMR gene diversity comparison
- Water bodies (lakes, drains) vs. built environments (houses)
- Focus on clinically relevant resistance genes and mobile genetic elements (MGEs)
- **Tools:** ABRicate, ResFinder, MOB-suite, R

---

### 🧫 Comparative Genomics — *Kocuria palustris* & *Deinococcus*
> Functional comparison across MAGs and reference genomes
- **Tools:** Prokka, DRAM, OrthoFinder, iTOL, R, Panaroo

---

### 🧬 antiSMASH for MAGs — Biosynthetic Gene Cluster (BGC) Prediction
> BGC annotation across metagenome-assembled genomes

- Summary pipeline for running and parsing antiSMASH results across large MAG sets
- **Tools:** antiSMASH, Python, R

---

## 🛠️ Skills & Tools

| Category | Tools |
|---|---|
| **Amplicon analysis** | QIIME2, DADA2, phyloseq |
| **Shotgun metagenomics** | Kraken2, Bracken, HUMAnN3, MetaPhlAn |
| **Assembly & binning** | MetaSPAdes, MetaBAT2, MaxBin2, CONCOCT |
| **MAG QC & dereplication** | CheckM2, dRep, GUNC |
| **Annotation** | Prokka, DRAM, antiSMASH, GTDB-Tk |
| **AMR & virulence** | ABRicate, ResFinder, AMRFinderPlus, VFDB |
| **Phylogenetics** | IQ-TREE2, FastTree, iTOL |
| **Workflow** | Snakemake, Nextflow, SLURM |
| **Data analysis & visualization** | R (vegan, ggplot2, phyloseq, microbiome), Python |

---

## 📊 Languages

![R](https://img.shields.io/badge/R-276DC3?style=flat-square&logo=r&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![Bash](https://img.shields.io/badge/Bash-4EAA25?style=flat-square&logo=gnu-bash&logoColor=white)
![Snakemake](https://img.shields.io/badge/Snakemake-00ADD8?style=flat-square)
![Nextflow](https://img.shields.io/badge/Nextflow-00BCD4?style=flat-square)

---

## 📫 Connect

- 🔗 [LinkedIn](#) ← *www.linkedin.com/in/saraswati-awasthi-16720b119*
- 🧬 [ORCID](#) ← *(https://orcid.org/0009-0000-6329-5158)*
- 📧 [Email](#) ← *saraswati.awasthi22@gmail.com*

---

*Open to freelance bioinformatics collaborations — microbiome data analysis, ARG profiling, MAG recovery, and metagenomic pipeline development.*
